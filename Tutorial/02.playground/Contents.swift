...
/*:
 ...
 When we see parenthesis in programming languages we often think of invoking something. When we see curly braces `{ }` we often think of structure like a class, if statement, or struct. Since we are denoting structure here, it makes more sense to use curly braces rather than parenthesis. How can we do this?
 
 Thunks and Swift's trailing closure syntax!
 
 Thunks are functions that take no arguments and return a value. It is something that has "already been thinked". Swift's trailing closure syntax means that when we have a function that takes another function as its last argument we can get rid of the parenthesis and just use curly braces.
